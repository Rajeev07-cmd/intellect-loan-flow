...
export default function CamGenerator() {
  ...
  const [aiCamData, setAiCamData] = useState<any>(null);

  const handleAIGenerate = async () => {
    if (!selectedApplication) return;
    const isUUID = /^[0-9a-f]{8}-/i.test(selectedApplication.id);
    if (!isUUID) {
      toast({ title: "Demo Application", description: "AI CAM generation only works with database applications." });
      return;
    }
    setGenerating(true);
    setGenComplete(false);
    try {
      const cam = await generateCam(selectedApplication.id);
      setAiCamData(cam);
      toast({ title: "AI CAM Generated", description: "Credit Appraisal Memo generated by AI successfully." });
      setGenComplete(true);
      setTimeout(() => setGenComplete(false), 5000);
    } catch (e: any) {
      toast({ title: "CAM Generation Failed", description: e.message || "Could not generate CAM report.", variant: "destructive" });
    } finally {
      setGenerating(false);
    }
  };
